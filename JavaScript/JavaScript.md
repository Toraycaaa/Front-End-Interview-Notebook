...
判断方法：Object.prototype.toString.call(obj)
...
先说 [] + {} 。一个数组加一个对象。加法会进行隐式类型转换，规则是调用其 valueOf() 或 toString() 以取得一个非对象的值（primitive value）。如果两个值中的任何一个是字符串，则进行字符串串接，否则进行数字加法。

[] 和 {} 的 valueOf() 都返回对象自身，所以都会调用 toString()，最后的结果是字符串串接。[].toString() 返回空字符串，({}).toString() 返回“[object Object]”。最后的结果就是“[object Object]”。

然后说 {} + [] 。看上去应该和上面一样。但是 {} 除了表示一个对象之外，也可以表示一个空的 block。在 [] + {} 中，[] 被解析为数组，因此后续的 + 被解析为加法运算符，而 {} 就解析为对象。但在 {} + [] 中，{} 被解析为空的 block，随后的 + 被解析为正号运算符。即实际上成了：{ // empty block }+[]
...
正常情况下，函数执行完成后，内部的变量会被销毁，内存空间被释放；

闭包，内部函数没有执行完成，外部函数变量还在内存栈中，没有被销毁；

闭包有两个常用的用途。

闭包的第一个用途是使我们在函数外部能够访问到函数内部的变量。通过使用闭包，我们可以通过在外部调用闭包函数，从而在外部访问到函数内部的变量，可以使用这种方法来创建私有变量。

函数的另一个用途是使已经运行结束的函数上下文中的变量对象继续留在内存中，因为闭包函数保留了这个变量对象的引用，所以这个变量对象不会被回收。（封装局部变量和函数，利用闭包和自调用函数实现模块化）
...
[《从event loop到async await来了解事件循环机制》](https://juejin.cn/post/6844903740667854861#heading-4)
[《Event Loop 原来是这么回事》](https://juejin.cn/post/6844903758661419016)
...
- 1.声明的变量只在声明时的代码块内有效
- 2.不存在声明提升
- 3.存在暂时性死区，如果在变量声明前使用，会报错（能够读取，但不能使用变量的区域就是暂存死区。在一个作用域中声明一个变量，暂存死区，指的就是作用域开始位置，到变量声明位置结束，引用暂存死区中的变量就会抛出引用错误。）
- 4.不允许重复声明，重复声明会报错
...
```
#### 175.为什么在for in循环中可以用const，和使用let有什么区别
```js
for (const s of arr) {
	console.log(s);
}
...
详细资料可以参考：
[《ES6 中的let 和 const》](https://www.cnblogs.com/SamWeb/p/10659352.html)

#### 176.为什么 [] ==false, [] == ![], [] != {}

[《JS的隐式转换 从 [] ==false 说起》](https://juejin.cn/post/6844903518311186440)

#### 177.哪些数组方法改变了原数组？数组都有什么方法

splice(开始index, 剪切个数), shift, unshift, reverse, sort, copyWithin(),


slice[开始index，结束index), map,  不会改变原数组

#### 178.JavaScript之（|、||、&、&&）操作符

```js
JavaScript中与java中的 位操作符 功能不同。java中位运算符可以操作 数值型 和 布尔型（非短路逻辑操作）；而JS中只能当做 数值 进行位运算，与^(按位异或)、 ~(按位非)、 <<(左移)、 >>(右移)、 >>>(无符号右移)功能一样，若操作数为非数值，则转换为数值。（数值转换规则不在本文阐述）
...
#### 179.typeof 与 instanceof区别

`typeof` 可以判断值类型, 返回字符串
...
[typeof和instanceof区别](https://github.com/febobo/web-interview/issues/65)
