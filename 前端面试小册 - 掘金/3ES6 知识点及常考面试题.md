...
```js
var a = 1 // var 是挂载到 window 上的，可以用 window.a 调用
let b = 1
const c = 1
...
![](https://cdn.jsdelivr.net/gh/huxingyi1997/my_img/img/20210626213007.png)
...
众所周知js属于**解释型语言**，它是执行的时候才会编译，是可以在解释器中直接执行的语言，不需要编译；js在执行时分为三个阶段，**语法检测**，**预编译**和**解释执行**。

- **语法检测阶段**：检查代码有无语法错误
- **预编译阶段**：**创建上下文对象，var声明的变量作为上下文属性**，用undefined进行初始化，function声明的函数也作为上下文对象，但是是保持了定义内容的。另外，**function声明的优先级是高于var的**。这就是var和function的变量提升
- **解释执行**：在解释执行过程中，变量在解析时先按照上下文对象寻找，如果没有则按照上下文对象的原型链寻找，还寻找不到就会去上级作用域寻找。
...
1. 首先**编译器**编译`var a = 2;`时，会查找当前作用域是否存在变量`a`，发现`a`并不存在，声明变量`a`。
2. 再由**引擎**执行由编译器生成的代码，此时变量`a`已经被**编译器**声明,但是还未被赋值，所以此时控制台输出的值是`undefined`。
3. `a = 2`，此时变量a的值为2。

[《深入理解JavaScript之变量提升规则》](https://juejin.cn/post/6844903976765227016)

[《js变量提升与编译原理》](https://www.jianshu.com/p/63519d0c7c6c)
...
![img](https://cdn.jsdelivr.net/gh/huxingyi1997/my_img/img/20210626213314.png)
...
Child.prototype = Object.create(Parent.prototype, { // 核心：不需要继承parent自身的属性
  // Object.create()的第二个参数定义了constructor函数的属性描述
  constructor: {
    value: Child,
    enumerable: false,
    writable: true,
...
![img](https://cdn.jsdelivr.net/gh/huxingyi1997/my_img/img/20210626213335.png)
...
当然这是简单版的响应式实现，如果需要实现一个 Vue 中的响应式，需要我们在 `get` 中收集依赖，在 `set` 派发更新，之所以 Vue3.0 要使用 `Proxy` 替换原本的 API (Object.defineProperty()) 原因在于 `Proxy` 无需一层层递归为每个属性添加代理，一次即可完成以上操作，性能上更好，并且原本的实现有一些数据更新不能监听到，但是 `Proxy` 可以完美监听到任何方式的数据改变，唯一缺陷可能就是浏览器的兼容性不好了。
...
```js
['1','2','3'].map(parseInt) // parseInt 的第二个参数是进制数
```
